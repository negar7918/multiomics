...
def main(args):
    ...
    folder = '0.0007_0.0005_5_21'# loss_min['config'] #
    desired_path = os.path.join(path, folder)
    data = np.load(desired_path + '/data_{}.npy'.format(disease))
    label = np.load(desired_path + '/label_{}.npy'.format(disease), allow_pickle=True)
    s = int(folder[-4]) # the number of groups
    ...
    view_shared_common = (view1_shared_em_new + .3 * view2_shared_em_new + .3 * view3_shared_em_new) / 3
    final_embedding = torch.cat((view1_specific_em_new, view2_specific_em_new, view3_specific_em_new, view_shared_common), dim=1)
    ...
    def plot_with_path(data, label, path, method):
        ...
        np.savetxt(method+"_umap_data.csv", df, delimiter=",")
        ...
        if 'tcga_brca' in path:
            for t, l in zip(g.legend_.texts, ["stage 1", "stage 2", "stage 3", "stage 4"]):
                t.set_text(l)
        plt.title(method, fontsize=20)
        plt.savefig(path + '_umap.png')
        plt.close()

    plot_with_path(data, truth, desired_path + "/data", 'Raw Data')
    plot_with_path(final_embedding, truth, desired_path + "/final_em", 'OMIDIENT')
